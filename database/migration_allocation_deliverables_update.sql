...
-- 2. Change shoot_sessions to TEXT (stores JSON array of session types), remove shoot_hours (derived from session type)
-- 3. Replace 4 community columns with single community_management_frequency
-- 4. Add ads_pre_ad_report and ads_post_ad_report columns
-- 5. Remove old ads columns (campaign_setup, campaign_monitoring, optimization, reporting)

-- Clear existing shoot_sessions data before type change
UPDATE project_allocation_sheets SET shoot_sessions = NULL;
...
ALTER TABLE project_allocation_sheets
  ...
  MODIFY COLUMN shoot_sessions TEXT DEFAULT NULL;

ALTER TABLE project_allocation_sheets
  ADD COLUMN community_management_frequency VARCHAR(50) DEFAULT 'Daily' AFTER community_lead_escalation;
...
ALTER TABLE project_allocation_sheets
  DROP COLUMN shoot_hours,
  DROP COLUMN community_dm_monitoring,
  DROP COLUMN community_comment_monitoring,
  DROP COLUMN community_review_monitoring,
  DROP COLUMN community_lead_escalation,
